...
Y.PriceCalendar = Y.extend(Calendar, Y.Base, {
     ...
     */ 
    renderUI: function() {
        Y.one('#calendarBox').append(this._initCalendarHTML(this.get('date')));
    ...
    _setWidth: function() {
        (function(that, boundingBox, contentBox) {
            boundingBox.all('.inner').setStyle('width', 520);
            boundingBox.all('table').setStyle('width', 520);
            boundingBox.setStyle('width', 1180);
...
}, 
{
     ...
     * @static
     */
    CALENDAR_TEMPLATE: '<div class="calendarBox">' +
                            '<div id="{bounding_box_id}" class="price-calendar-bounding-box">' +
                                '<div class="container">' +
    ...
    BOTTOM_TEMPLATE: '<p class="btns">' +
                        '<input type="button" value="\u786e\u5b9a" class="confirm-btn" />' +
                        '<input type="button" value="收回" class="close-btn" />' +
                    '</p>',
    /**
     * 日历组件标识
     *
